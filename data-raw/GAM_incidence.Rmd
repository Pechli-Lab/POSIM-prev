...
popON_M <- filter(StatCanpop_PSDtransfer, GEO == "Ontario", Sex == "Males")
...
#####
popON_F <- filter(StatCanpop_PSDtransfer, GEO == "Ontario", Sex == "Females")
...
### BOTH SEXES
popON <- filter(StatCanpop_PSDtransfer, GEO == "Ontario", Sex == "Both sexes")
...
#create childpop_M_Y, childpop_F_Y, and childpop_Y
```{r}
...
incidence_backup_Aug1621 <- read.csv("REDACTED")
incidence <- as.data.frame(incidence_backup_Aug1621)

incidence <- filter(incidence, dxyear > 1984) #running gam models on pogo data only.
...
#4. RMSE results for 5-year ahead TSCV
```{r}
...
write.csv(RMSEresults_bind_tscv5, "RMSEresults_allmodels_50to40_Mar922.csv") #prior to manual selection**
...
#Inspect pdfallGAMfits_1950to2040_Mar922.pdf to determine changes. manually choose model for select groups where initially chosen one was not most appropriate - March9 2022

#Females
RMSEresults_min_tscv5$modelno[RMSEresults_min_tscv5$group == "ALL_Female"] <- 5 #was 6 prior
RMSEresults_min_tscv5$manualselection[RMSEresults_min_tscv5$group == "ALL_Female"] <- "Yes"

RMSEresults_min_tscv5$modelno[RMSEresults_min_tscv5$group == "Neuroblastoma_Female"] <- 5 #was 4 prior
RMSEresults_min_tscv5$manualselection[RMSEresults_min_tscv5$group == "Neuroblastoma_Female"] <- "Yes"

RMSEresults_min_tscv5$modelno[RMSEresults_min_tscv5$group == "Renal tumours_Female"] <- 2 #was 1 prior
RMSEresults_min_tscv5$manualselection[RMSEresults_min_tscv5$group == "Renal tumours_Female"] <- "Yes"

RMSEresults_min_tscv5$modelno[RMSEresults_min_tscv5$group == "Hepatic tumours_Female"] <- 3 #was 1 prior
RMSEresults_min_tscv5$manualselection[RMSEresults_min_tscv5$group == "Hepatic tumours_Female"] <- "Yes"

RMSEresults_min_tscv5$modelno[RMSEresults_min_tscv5$group == "Soft tissue sarcomas_Female"] <- 5 #was 4 prior
RMSEresults_min_tscv5$manualselection[RMSEresults_min_tscv5$group == "Soft tissue sarcomas_Female"] <- "Yes"

RMSEresults_min_tscv5$modelno[RMSEresults_min_tscv5$group == "Other epithelial and unspecified neoplasms_Female"] <- 5 #was 2 prior
RMSEresults_min_tscv5$manualselection[RMSEresults_min_tscv5$group == "Other epithelial and unspecified neoplasms_Female"] <- "Yes"

#### Male
RMSEresults_min_tscv5$modelno[RMSEresults_min_tscv5$group == "AML and other leukemias_Male"] <- 5 #was 4 prior
RMSEresults_min_tscv5$manualselection[RMSEresults_min_tscv5$group == "AML and other leukemias_Male"] <- "Yes"

RMSEresults_min_tscv5$modelno[RMSEresults_min_tscv5$group == "Non-Hodgkin lymphomas and other lymphomas_Male"] <- 4 #was 1 prior
RMSEresults_min_tscv5$manualselection[RMSEresults_min_tscv5$group == "Non-Hodgkin lymphomas and other lymphomas_Male"] <- "Yes"

RMSEresults_min_tscv5$modelno[RMSEresults_min_tscv5$group == "Neuroblastoma_Male"] <- 2 #was 1 prior
RMSEresults_min_tscv5$manualselection[RMSEresults_min_tscv5$group == "Neuroblastoma_Male"] <- "Yes"

RMSEresults_min_tscv5$modelno[RMSEresults_min_tscv5$group == "Hepatic tumours_Male"] <- 2 #was 3 prior
RMSEresults_min_tscv5$manualselection[RMSEresults_min_tscv5$group == "Hepatic tumours_Male"] <- "Yes"

RMSEresults_min_tscv5$modelno[RMSEresults_min_tscv5$group == "Bone tumours_Male"] <- 6 #was 3 prior
RMSEresults_min_tscv5$manualselection[RMSEresults_min_tscv5$group == "Bone tumours_Male"] <- "Yes"

RMSEresults_min_tscv5$modelno[RMSEresults_min_tscv5$group == "Germ cell tumours_Male"] <- 5 #was 6 prior
RMSEresults_min_tscv5$manualselection[RMSEresults_min_tscv5$group == "Germ cell tumours_Male"] <- "Yes"

RMSEresults_min_tscv5$modelno[RMSEresults_min_tscv5$group == "Other epithelial and unspecified neoplasms_Male"] <- 5 #was 4 prior
...
write.csv(RMSEresults_min_tscv5, "RMSEresults_selectedmodels_50to40_Mar922.csv")
...
#export plots of plotsums
```{r}

plotsums_bind <- ldply(plotrates_list, data.frame) #contains all plotrates_all dfs
colnames(plotsums_bind) <- c("group", "P", "rate", "A", "status", "lograte", "logSE", "SE", "lower", "upper")
plotsums_bind$id <- rep(1:14, each = nrow(plotrates_list[["ALL_Female"]][["plotrates_all"]]), times = 2)

plotsums_bind$id <- factor(plotsums_bind$id,
                           levels = c(1:14),
                           labels = c("ALL", "AML and other leukemias", "Hodgkin lymphomas", "Non-Hodgkin lymphomas and other lymphomas", "Astrocytoma", "Other CNS neoplasms", "Neuroblastoma", "Retinoblastoma", "Renal tumours", "Hepatic tumours", "Bone tumours", "Soft tissue sarcomas", "Germ cell tumours", "Other epithelial and unspecified neoplasms"))

#plotsums_bind_tscv5 <- plotsums_bind #saving a version when running above chunk with tscv5 results (to make combined plots later)
#plotsums_bind_tscv1 <- plotsums_bind
 
plotsums_bind_cancertype <- aggregate(rate ~ P + id + status, data = plotsums_bind, FUN = sum)

#merge with empty df where we can have NA for observed rates in future years - to get plot to run
P <- rep(1950:2040, each = 14, times = 1)
emptydf <- as.data.frame(P)
groupnames <- c("ALL", "AML and other leukemias", "Hodgkin lymphomas", "Non-Hodgkin lymphomas and other lymphomas", "Astrocytoma", "Other CNS neoplasms", "Neuroblastoma", "Retinoblastoma", "Renal tumours", "Hepatic tumours", "Bone tumours", "Soft tissue sarcomas", "Germ cell tumours", "Other epithelial and unspecified neoplasms")
emptydf$id <- rep(groupnames, each = 1, times = length(1950:2040))
emptydf$status <- rep("observed")
emptydf$rate <- rep(NA)
emptydf <- emptydf %>% filter(P <= 1984 & P >= 2020) #remove 1985-2019 as we have observed data already

plotsums_bind_cancertype_bind <- rbind(plotsums_bind_cancertype, emptydf)

pdf("plotsums_cancertype_tscv5_50to40_Mar922.pdf")

for(i in 1:4){
  print((ggplot(plotsums_bind_cancertype_bind, aes(x = P, y = rate)) + geom_point(aes(color=status)) + labs(title = "Incidence rates summed for all sexes and ages - by cancer type", subtitle = "Models selected using tsCV with multi-year forecasting", x = "Year", y = "Incidence rate per million") + theme(legend.position = "bottom")) + facet_wrap_paginate(plotsums_bind_cancertype_bind$id, ncol = 2, nrow = 2, page = i))
}

for(i in 1:4){
  print((ggplot(plotsums_bind_cancertype_bind, aes(x = P, y = rate)) + geom_point(aes(color=status)) + labs(title = "Incidence rates summed for all sexes and ages - by cancer type. Free Y axis", subtitle = "Models selected using tsCV with multi-year forecasting", x = "Year", y = "Incidence rate per million") + theme(legend.position = "bottom")) + facet_wrap_paginate(plotsums_bind_cancertype_bind$id, ncol = 2, nrow = 2, page = i, scales = "free_y"))
}
dev.off()

#######
plotsums_bind_all <- aggregate(rate ~ P + status, data = plotsums_bind, FUN = sum)

#merge with empty df again
P <- rep(1950:2040, each = 1, times = 1)
emptydf <- as.data.frame(P)
emptydf$status <- rep("observed")
emptydf$rate <- rep(NA)
emptydf <- emptydf %>% filter(P <= 1984 & P >= 2020) #remove 1985-2019 as we have observed data already
...
plotsums_bind_all_bind <- rbind(plotsums_bind_all, emptydf)

pdf("plotsums_all_tscv5_50to40_Mar922.pdf")
print(ggplot(plotsums_bind_all_bind, aes(x = P, y = rate)) + geom_point(aes(color=status)) + labs(title = "Overall incidence rates: summed for all sexes, ages, and cancer types", x = "Year", y = "Incidence rate per million") + theme(legend.position = "bottom"))
dev.off()
