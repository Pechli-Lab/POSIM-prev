...
sum_decade_time <- (length(0:70)+length(0:61)+length(0:51)+length(0:41)+length(0:31)+length(0:21)+length(0:11))
...
decade_strata <- rep(decade_dx, times = c(length(0:70), length(0:61), length(0:51), length(0:41), length(0:31), length(0:21),length(0:11)))
...
Yeh_df_test <- reshape2::melt(Yeh_df, id.vars=c("Decade_start", "Decade_stop", "Cohort"))

Yeh_mean <- Yeh_df_test %>% filter(substring(variable, first = 8) == "log")
...
Yeh_se <- Yeh_df_test %>% filter(substring(variable, first = 8) == "SE")
...
k = 0
RRmort <- array(dim = c(length(min(Yeh_mean$Age):max(Yeh_mean$Age)), decades, n_sim), dimnames = list(min(Yeh_mean$Age):max(Yeh_mean$Age), unique(Yeh_mean$Decade_start), 1:n_sim))
for (j in 1:decades){
  for (i in 1:length(min(Yeh_mean$Age):max(Yeh_mean$Age))){
    k = k+1
    RRmort[i,j,] <- rlnorm(n_sim, Yeh_mean$value[[k]], Yeh_se$value[[k]])
    
  }
}
RRmort_final <- array(dim = c(length(min(Yeh_mean$Age):max(Yeh_mean$Age)), length(init_year:max(Yeh_mean$Decade_stop)), n_sim), dimnames = list(min(Yeh_mean$Age):max(Yeh_mean$Age), init_year:max(Yeh_mean$Decade_stop), 1:n_sim))
RR_long <- list()
...
prev_sim <- matrix(NA, nrow = length(init_year:max_year), ncol = n_sim) #1970-2040 prevalence
prev_rate <- matrix(NA, nrow = length(init_year:max_year), ncol = n_sim) #1970-2040 prevalence rate per 100,000
prev_imm <- matrix(NA, nrow = length(init_year:max_year), ncol = n_sim) #1970-2040 prevalence - identify proportion diagnosed out of country
prev_bornON <- matrix(NA, nrow = length(init_year:max_year), ncol = n_sim) #born and diagnosed in Ontario
prev_imm_dxON <- matrix(NA, nrow = length(init_year:max_year), ncol = n_sim) #born outside Ontario, diagnosed in Ontario
prev_sim_true_prev <- matrix(NA, nrow = length(init_year:max_year), ncol = n_sim) #1990-2019 ICES prevalence comparison
prev_sim_true_prev_70 <- matrix(NA, nrow = length(init_year:max_year), ncol = n_sim) #1970-2019 ICES prevalence comparison
...
prev_sim_ICCC <- matrix(NA, nrow = length(init_year:max_year)*length(ICCCgroups), ncol = n_sim) #1970-2040 prevalence by cancer type
prev_rate_ICCC <- matrix(NA, nrow = length(init_year:max_year)*length(ICCCgroups), ncol = n_sim) #1970-2040 prevalence rate per 100,000 by cancer type
dxyear_sim_imm <- matrix(NA, nrow = length((init_year+1):max_year), ncol = n_sim) #annual overall incidence counts - ON-born individuals + immigrants diagnosed in ON
dxyear_sim_imm_rate <- matrix(NA, nrow = length((init_year+1):max_year), ncol = n_sim) #annual overall incidence rates - ON-born individuals + immigrants diagnosed in ON
dxyear_sim_imm5_df <- matrix(NA, nrow = length(fiveyrperiods), ncol = n_sim) #incidence counts overall by 5-year time periods
dxyear_sim_imm5_rate_df <- matrix(NA, nrow = length(fiveyrperiods), ncol = n_sim) #incidence rate overall by 5-year time periods
dxyeargroup_sim <- matrix(NA, nrow = length((init_year+1):max_year)*length(ICCCgroups), ncol = n_sim) #incidence counts by cancer type
dxyeargroup_iccc_df <- matrix(NA, nrow = length(ICCCgroups)*length(fiveyrperiods), ncol = n_sim) #incidence counts in 5-year time periods by cancer type
dxyeargroup_iccc_rate_df <- matrix(NA, nrow = length(ICCCgroups)*length(fiveyrperiods), ncol = n_sim) #incidence rates in 5-year time periods by cancer type
attainedage_year_df_m <- matrix(NA, nrow = length(attainedage_group)*(length(c(start_year_pred,max_year))), ncol = n_sim) # prevalence by attained age for 2020 and 2040
attainedage_ICCC_m <- matrix(NA, nrow = length(attainedage_group)*length(ICCCgroups)*(length(c(start_year_pred,max_year))), ncol = n_sim) #prevalence by attained age and cancer type for 2020 and 2040
survstatus_df <- matrix(NA, nrow = length(survstatus_group)*(length(c(start_year_pred,max_year))), ncol = n_sim) #prevalence by FU in 5 yr periods for 2020, 2040
...
surv_kk_HL <- matrix(NA,nrow = sum_decade_time, ncol = n_sim)
surv_kk_AST <- matrix(NA,nrow = sum_decade_time, ncol = n_sim)
surv_kk_OCNS <- matrix(NA,nrow = sum_decade_time, ncol = n_sim)
surv_kk_NEU <- matrix(NA,nrow = sum_decade_time, ncol = n_sim)
surv_kk_RET <- matrix(NA,nrow = sum_decade_time, ncol = n_sim)
surv_kk_REN <- matrix(NA,nrow = sum_decade_time, ncol = n_sim)
surv_kk_HEP <- matrix(NA,nrow = sum_decade_time, ncol = n_sim)
surv_kk_BONE <- matrix(NA,nrow = sum_decade_time, ncol = n_sim)
surv_kk_SAR <- matrix(NA,nrow = sum_decade_time, ncol = n_sim)
surv_kk_GERM <- matrix(NA,nrow = sum_decade_time, ncol = n_sim)
...
sim_results <- foreach (k = 1:n_sim) %dopar% {
  ...
  popm.mean <- ON.sim_demog$male[,,k] #mean annual pop estimates by sex
  popf.mean <- ON.sim_demog$female[,,k]
  ...
  popON_m_pred <- popON_m_pred %>% 
    ...
    mutate(Sex = rep("male"),
           ...
           Pop = round(Pop), #AM: changed from ceiling to round
  ...
  row.names(pop.ON_f) <- c(0:max_age) #assign matrix rows to correspond with the age range (instead of 1-111)
  ...
  for(i in 1:ncol(pop.ON_f)){
    ...
    test2[[i]] <- diff(diag(help.dat))
    year2[[i]] <- colnames(help.dat)[-1]
    age2[[i]] <- rownames(help.dat)[-1][1:length(test2[[i]])]
    
  }
  
  results2 <- cbind(unlist(year2),  unlist(age2), unlist(test2[-length(test2)]))
  ...
  for(i in 1:nrow(pop.ON_f)){
    ...
    test3[[i]] <- diff(diag(help.dat))
    year3[[i]] <- colnames(help.dat)[-1][1:length(test3[[i]])]
    age3[[i]] <- rownames(help.dat)[-1]
    
  }
  
  results3 <- cbind(unlist(year3),  unlist(age3), unlist(test3[-length(test3)]))
  ...
  popON_net_f <- dplyr::union(results2, results3) #union - rows that appear in x or y. (Duplicates removed)
  ...
  #this df now contains the pop to simulate at each year within the microsim (# who enters (+) or exits (-) by year, age and sex)
  popON_net_m <- dplyr::union(results2, results3) #union - rows that appear in x or y. (Duplicates removed)
  ...
  v_n_popsim_male <- popON_net %>%
    # dplyr::filter(sex == "male" & year > init_year & age == 0 & pop > 0)
  ...
  Probs <- function(M_t, df_X0, t, kk) {
    ...
    p_NCD_RR   <- left_join(df_X0, RR_long[[k]], by = c("dxyear", "age_current"))[,"RR"]
  ...
  outcomes <- MicroSim(n_i, df_X =df_X, seed = k*b*100, k)
  ...
  #prev_rate[,k] <- matrix((prev_sim[,k] / prev_pop$pop)*10^5)
  #prevpop_PSA[,k] <- prev_pop$pop
  ...
  #prev_imm[,k] <- colSums(m_M_prev_imm == "C" | m_M_prev_imm == "CRE", na.rm= T)*adj_fac
  ...
  #prev_bornON[,k] <- colSums(m_M_prev_bornON == "C" | m_M_prev_bornON == "CRE", na.rm= T)*adj_fac
  ...
  #prev_imm_dxON[,k] <- colSums(m_M_prev_imm_dxON == "C" | m_M_prev_imm_dxON == "CRE", na.rm= T)*adj_fac
  ...
  #prev_sim_true_prev[,k] <- colSums(m_M_true_prev == "C" | m_M_true_prev == "CRE", na.rm= T)*adj_fac
  ...
  #1992-onward CCR prevalence (get estimate for 2018 and project forward for the grant) (all cancers combined)
  true_prev_id_CCR <- which(outcomes$df_X$dxyear >= 1992 & outcomes$df_X$dxyear >= outcomes$df_X$year_imm)
  m_M_true_prev_id_CCR <- outcomes$m_M[true_prev_id_CCR,]
  CCRprev <- colSums(m_M_true_prev_id_CCR == "C" | m_M_true_prev_id_CCR == "CRE", na.rm= T)*adj_fac
  ...
  prev_sim_true_prev_ICCC <- microsim_trueprev_ICCC_all$prev_sim
  ...
  prev_sim_true_prev_70_ICCC <- microsim_trueprev_70_ICCC_all$prev_sim
  
  
  #####################################################
  #1992-2017 CCR prevalence comparison - by ICCC group
  
  tomodel1  <- outcomes$df_X %>% dplyr::count(ICCCgroup) %>% filter(is.na(ICCCgroup) !=1)
  
  CCR_ICCC <- vector(mode = "list")
  for(i in 1:dim(tomodel1)[1]){
    
    true_prev_ICCC <- which(outcomes$df_X$dxyear >= 1992 & outcomes$df_X$dxyear <= 2017 & outcomes$df_X$ICCCgroup == tomodel1$ICCCgroup[i] & outcomes$df_X$dxyear >= outcomes$df_X$year_imm)
    m_M_true_prev_ICCC <- outcomes$m_M[true_prev_ICCC,]
    prev_sim_true_prev_byICCC <- colSums(m_M_true_prev_ICCC == "C" | m_M_true_prev_ICCC == "CRE", na.rm= T)*adj_fac
    
    prev_ICCC_df <- data.frame(prev_sim = prev_sim_true_prev_byICCC)
    
    CCR_ICCC[[i]] <- prev_ICCC_df
    names(CCR_ICCC)[i] <- paste0(tomodel1$ICCCgroup[i])
  }
  
  CCR_ICCC_all <- ldply(CCR_ICCC, data.frame)
  CCR_ICCC_counts <- CCR_ICCC_all$prev_sim
  ...
  prev_sim_ICCC <- microsimprev_ICCC_all$prev_sim
  ...
  #dxyear_sim_imm[,k] <- dxyear_sim_imm_df$n_adj
  #dxyear_sim_imm_rate[,k] <- dxyear_sim_imm_df$rate
  ...
  #dxyear_sim_imm5_df[,k] <- dxyear_sim_imm5$n_adj
  ...
  #dxyear_sim_imm5_rate_df[,k] <- dxyear_sim_imm5_rate$rate
  ...
  #dxyeargroup_sim[,k] <- dxyeargroup_sim_df$n_adj
  ...
  #dxyeargroup_iccc_df[,k] <- dxyeargroup_iccc$n_adj
  #dxyeargroup_iccc_rate_df[,k] <- dxyeargroup_iccc$rate
  ...
  #attainedage_year_df_m[,k] <- attainedage_year_df$n
  ...
  #survstatus_df[,k] <- survstatusdf$n
  ...
  #survstatus_df2[,k] <- survstatusdf2$n
  ...
  #survstatus_ICCC_m[,k] <- survstatus_ICCC$n
  ...
  #survstatus2_ICCC_m[,k] <- survstatus2_ICCC$n
  ...
  # surv_M <- rowSums(cancer_m =="C") +
  #           rowSums(cancer_m =="CRE")
  # fup_M <- cancer_m[,ncol(cancer_m)]
  # event_M <- fup_M =="dead" |fup_M =="candead"
  ...
  # cancer_df_X$dead =event_M 
  # cancer_df_X$t_cancer =surv_M
  ...
  #CCR prevalence approximation for CIHR grant
  #diagnosed between 1992-2015 in Ontario (no prior cases counted from net immigrants)
  prev_CCR_age <- which(outcomes$df_X$dxyear >= 1992 & outcomes$df_X$dxyear <= 2015 & outcomes$df_X$dxyear >= outcomes$df_X$year_imm)
  m_M_prev_CCR_age <- outcomes$m_M[prev_CCR_age,]
  m_M_prev_CCR_age[m_M_prev_CCR_age == "tobeborn"] = 0
  m_M_prev_CCR_age[m_M_prev_CCR_age == "noC"] = 0
  m_M_prev_CCR_age[m_M_prev_CCR_age == "outON"] = 0
  m_M_prev_CCR_age[m_M_prev_CCR_age == "leftON"] = NA
  m_M_prev_CCR_age[m_M_prev_CCR_age == "candead"] = NA
  m_M_prev_CCR_age[m_M_prev_CCR_age == "dead"] = NA
  attainedage <- apply(m_M_prev_CCR_age, 1, function(x){ y = x != "0"; cumsum(y)})
  attainedage <- t(attainedage)
  attainedage[attainedage == 0] <- NA
  attainedage2 <- outcomes$df_X[prev_CCR_age,]
  colnames(attainedage) <- init_year:max_year
