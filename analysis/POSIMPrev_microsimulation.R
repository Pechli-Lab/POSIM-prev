...
##First create a holding folder called 'microsim_output'
# dir.create(paste0("~/GitHub/POSIM-prev/analysis/","microsim_output"))

##500/9 - need 55.5 runs of 9 sims to get to 500 iterations. Create 56 folders
# batch <- c(1:56)
# for (b in batch){
#   folder<-dir.create(paste0("~/GitHub/POSIM-prev/analysis/microsim_output/","batch",b,"rawdataoutput"))
# }
...
con.name  <- "Canada"
StatCan_start_year <- 1971      # StatCan pop data starts at 1971, not 1970
StatCan_end_year <- 2020

init_year <- 1970               # starting year for the microsimulation model

start_year_pred <- 2020         # starting year for future time horizon (projections of births, population size, incident rates, etc.)
max_year  <- 2040               # end year for the microsimulation model
max_age   <- 100                # Max age that individuals in the cohort can reach
...
ICCCgroups <- c("ALL", "AML and other leukemias", "Astrocytoma", "Bone tumours", "Germ cell tumours", "Hepatic tumours", "Hodgkin lymphomas", "Neuroblastoma", "Non-Hodgkin lymphomas and other lymphomas", "Other CNS neoplasms", "Other epithelial and unspecified neoplasms", "Renal tumours", "Retinoblastoma", "Soft tissue sarcomas") #alphabetical order was used in multi-state survival modeling conducted at ICES
...
#mortality projections for all ages - 0-110+: for use in 'nocancer' and 'cancer' states
...
predrates$cancer_type <- as.factor(predrates$cancer_type) #prints in alphabetical order
ICCCgroups_incidence <- c("ALL", "AML and other leukemias", "Hodgkin lymphomas", "Non-Hodgkin lymphomas and other lymphomas", "Astrocytoma", "Other CNS neoplasms", "Neuroblastoma", "Retinoblastoma", "Renal tumours", "Hepatic tumours", "Bone tumours", "Soft tissue sarcomas", "Germ cell tumours", "Other epithelial and unspecified neoplasms") #reorder by ICCC-3 roman numeral groupings
...
#Calculate net migration in children to use inside the microsim's nocancer state
  ...
  Probs <- function(M_t, df_X0, t, kk) {
    ...
    for(trans in 1:sum(!is.na(m_t))){
      ...
      for(j in seq_along(ICCCgroups)){
        ...
        if(nrow(X_j) > 0){
          ...
          X_temp[,"dxyear"] =  X_temp[,"dxyear"] - 1989
          ...
          if(trans == 2){ #diagnosis to death
            ...
            X_temp_70to89[,"dxyear"] =  X_temp_70to89[,"dxyear"] - 1969
    ...
    #age under 15 where still at risk of developing cancer
    age_at_risk <- df_X0$age_current <= max_dx_age #ages at risk are below 15
  ...
  MicroSim <- function(n_i, df_X, seed = 1, kk) {
    ...
    for (t in 1:n_t) {
      ...
      m_M[df_X$year_imm == (init_year + t) & !is.na(df_X$year_death), ] = "outON" #if you've died before you immigrated, assign your entire trajectory to be out of country
  ...
  ################################ #
  #assign cancer types for last year of simulation
...
#################################################### #
  ...
  #1970-2039 incidence counts per year (all cancers combined)
  ...
  dxyear_sim_imm_df <- outcomes$df_X %>%
    filter(dxyear >= year_imm) %>% #to look at incidence diagnosed in Ontario (among ON-born residents and immigrants)
    group_by(dxyear) %>%
    dplyr::count() %>%
  ...
  #1970-2039 incidence counts per year by cancer type
  ...
  #1970-2039 incidence counts by cancer type, sex, age at diagnosis
  ...
  emptydf_inc_agesex <- filter(emptydf_inc_agesex, dxyear <= 2039)
  ...
  # prevalence by attained age for 2020 and 2040 (all cancers combined)
  ...
  #overall prevalence by attained age group
  ...
  #prevalence by attained age and cancer type  - 2020 and 2040
  tomodel1  <- outcomes$df_X %>% dplyr::count(ICCCgroup) %>% filter(is.na(ICCCgroup) !=1)
...
################################################################################ #  
  ...
  cancer_df_X$dead = ifelse(cancer_df_X$year_death < 9999, 1, 0)  
  cancer_df_X$cre = ifelse(cancer_df_X$year_cre < 9999, 1, 0) 
  
  cancer_df_X$t_cancer = cancer_df_X$v_dx0 + cancer_df_X$v_cre0
...
write.csv(prev_sim, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/prev_sim_kk.csv"), row.names = F)
write.csv(prev_rate, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/prev_rate_kk.csv"), row.names = F)
write.csv(prevpop_PSA, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/prevpop_PSA_kk.csv"), row.names = F)
write.csv(prev_imm, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/prev_imm_kk.csv"), row.names = F)
write.csv(prev_bornON, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/prev_bornON_kk.csv"), row.names = F)
write.csv(prev_imm_dxON, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/prev_imm_dxON_kk.csv"), row.names = F)
write.csv(prev_sim_true_prev, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/prev_sim_true_prev_kk.csv"), row.names = F)
write.csv(prev_sim_true_prev_70, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/prev_sim_true_prev_70_kk.csv"), row.names = F)
write.csv(prev_sim_true_prev_ICCC, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/prev_sim_true_prev_ICCC_kk.csv"), row.names = F)
write.csv(prev_sim_true_prev_70_ICCC, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/prev_sim_true_prev_70_ICCC_kk.csv"), row.names = F)
write.csv(prev_sim_ICCC, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/prev_sim_ICCC_kk.csv"), row.names = F)
write.csv(prev_rate_ICCC, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/prev_rate_ICCC_kk.csv"), row.names = F)
write.csv(prev_sim_char, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/prev_sim_char_kk.csv"), row.names = F)
write.csv(prev_sim_ICCC_char, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/prev_sim_ICCC_char_kk.csv"), row.names = F)
write.csv(childpop_PSA, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/childpop_PSA_kk.csv"), row.names = F)
write.csv(dxyear_sim_imm, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/dxyear_sim_imm_kk.csv"), row.names = F)
write.csv(dxyear_sim_imm_rate, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/dxyear_sim_imm_rate_kk.csv"), row.names = F)
write.csv(dxyear_sim_imm5_df, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/dxyear_sim_imm5_df_kk.csv"), row.names = F)
write.csv(dxyear_sim_imm5_rate_df, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/dxyear_sim_imm5_rate_df_kk.csv"), row.names = F)
write.csv(dxyeargroup_sim, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/dxyeargroup_sim_kk.csv"), row.names = F)
write.csv(dxyeargroup_sim_age, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/dxyeargroup_sim_age_kk.csv"), row.names = F)
write.csv(dxyeargroup_iccc_df, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/dxyeargroup_iccc_df_kk.csv"), row.names = F)
write.csv(dxyeargroup_iccc_rate_df, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/dxyeargroup_iccc_rate_df_kk.csv"), row.names = F)
write.csv(attainedage_year_df_m, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/attainedage_year_df_m_kk.csv"), row.names = F)
write.csv(attainedage_aspr_df_m, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/attainedage_aspr_df_m_kk.csv"), row.names = F)
write.csv(attainedage_ICCC_m, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/attainedage_ICCC_m_kk.csv"), row.names = F)
write.csv(attainedage_ICCC_aspr_m, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/attainedage_ICCC_aspr_m_kk.csv"), row.names = F)
write.csv(survstatus_df, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/survstatus_df_kk.csv"), row.names = F)
write.csv(survstatus_df2, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/survstatus_df2_kk.csv"), row.names = F)
write.csv(survstatus_ICCC_m, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/survstatus_ICCC_m_kk.csv"), row.names = F)
write.csv(survstatus2_ICCC_m, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/survstatus2_ICCC_m_kk.csv"), row.names = F)
write.csv(surv_kk, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_kk.csv"), row.names = F)
write.csv(surv_kk_ALL, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_ALL_kk.csv"), row.names = F)
write.csv(surv_kk_AML, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_AML_kk.csv"), row.names = F)
write.csv(surv_kk_NHL, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_NHL_kk.csv"), row.names = F)
write.csv(surv_kk_HL, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_HL_kk.csv"), row.names = F)
write.csv(surv_kk_AST, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_AST_kk.csv"), row.names = F)
write.csv(surv_kk_OCNS, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_OCNS_kk.csv"), row.names = F)
write.csv(surv_kk_NEU, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_NEU_kk.csv"), row.names = F)
write.csv(surv_kk_RET, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_RET_kk.csv"), row.names = F)
write.csv(surv_kk_REN, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_REN_kk.csv"), row.names = F)
write.csv(surv_kk_HEP, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_HEP_kk.csv"), row.names = F)
write.csv(surv_kk_BONE, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_BONE_kk.csv"), row.names = F)
write.csv(surv_kk_SAR, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_SAR_kk.csv"), row.names = F)
write.csv(surv_kk_GERM, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_GERM_kk.csv"), row.names = F)
write.csv(surv_kk_OTHER, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_OTHER_kk.csv"), row.names = F)
write.csv(surv_kk_ICCC_5, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_kk_ICCC_5.csv"), row.names = F)
write.csv(surv_kk_5, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_kk_5.csv"), row.names = F)
write.csv(surv_pfs_kk, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_pfs_kk.csv"), row.names = F)
write.csv(surv_pfs_kk_ICCC, paste0("~/GitHub/POSIM-prev/analysis/microsim_output/batch",b,"rawdataoutput/surv_pfs_kk_ICCC.csv"), row.names = F)
