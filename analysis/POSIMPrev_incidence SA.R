...
# dir.create(paste0("~/GitHub/POSIM-prev/analysis/","SA_inc_output"))

##250/9 - need 27.7 runs of 9 sims to reach 250 iterations. Create 28 folders
# batch <- c(1:28)
# for (b in batch){
#   folder<-dir.create(paste0("~/GitHub/POSIM-prev/analysis/SA_inc_output/","batch",b,"rawdataoutput"))
# }
...
#Scenario analysis with incidence rates held constant at 2019 values 
predrates_2019 <- predrates %>% filter(year == 2019)

predrates <- predrates %>% filter(year <= 2019)

predrates_SA <- bind_rows(replicate(21, predrates_2019, simplify = FALSE)) #repeat rates for 2019 for years 2020-2040
predrates_SA$year <- rep(start_year_pred:max_year, each = length(ICCCgroups)*length(sex_var)*length(dxages))

predrates <- rbind(predrates, predrates_SA) #predrates now contains constant rates for years 2020-2040
...
sim_results <- foreach (k = 1:n_sim) %dopar% {
  ...
  MicroSim <- function(n_i, df_X, seed = 1, kk) {
    ...
    for (t in 1:n_t) {
      ...
      check_sum_of_transition_array(m_P, n_states = sum(tobeallocated), n_t = n_t, verbose = TRUE) # check if each of the rows of the transition probabilities matrix sum to one
  ...
  # prev_pop_agesex <- pop %>% group_by(year, age, sex) %>% dplyr::summarise(pop = sum(pop)) %>% select(pop)
  # prevpop_agesex_PSA <- prev_pop_agesex$pop
  ...
  microsimprev_char_all_df[is.na(microsimprev_char_all_df$prev_sim)] <- 0
...
#################################################### #
  ...
  microsimprev_ICCC_char_df[is.na(microsimprev_ICCC_char_df$prev_sim)] <- 0
  ...
  #dxyeargroup_sim[,k] <- dxyeargroup_sim_age_df$n_adj
...
################################################################################ #  
  ...
  cancer_id <- which(is.na(outcomes$df_X$dxyear )==F)
  cancer_df_X <- outcomes$df_X[cancer_id,]
  #using m_M to capture time in cancer state doesn't account for people who were diagnosed out of country (still in outofON state) and then immigrated (resulting fup_M value is not large enough)
  # cancer_m   <- outcomes$m_M[cancer_id ,]
  ...
  cancer_df_X <- cancer_df_X %>%
    dplyr::group_by(ID) %>%
    dplyr::mutate(year_pfs_event = min(year_cre, year_death, na.rm = TRUE)) %>% #find the first occurring event of cre or death
  ungroup()
...
write.csv(dxyeargroup_sim_age, paste0("~/GitHub/POSIM-prev/analysis/SA_inc_output/batch",b,"rawdataoutput/dxyeargroup_sim_age_kk.csv"), row.names = F)
write.csv(dxyear_sim_imm, paste0("~/GitHub/POSIM-prev/analysis/SA_inc_output/batch",b,"rawdataoutput/dxyear_sim_imm_kk.csv"), row.names = F)
write.csv(dxyear_sim_imm_rate, paste0("~/GitHub/POSIM-prev/analysis/SA_inc_output/batch",b,"rawdataoutput/dxyear_sim_imm_rate_kk.csv"), row.names = F)

write.csv(prev_sim, paste0("~/GitHub/POSIM-prev/analysis/SA_inc_output/batch",b,"rawdataoutput/prev_sim_kk.csv"), row.names = F)
write.csv(prev_sim_char, paste0("~/GitHub/POSIM-prev/analysis/SA_inc_output/batch",b,"rawdataoutput/prev_sim_char_kk.csv"), row.names = F)
write.csv(prev_sim_ICCC_char, paste0("~/GitHub/POSIM-prev/analysis/SA_inc_output/batch",b,"rawdataoutput/prev_sim_ICCC_char_kk.csv"), row.names = F)

write.csv(attainedage_aspr_df_m, paste0("~/GitHub/POSIM-prev/analysis/SA_inc_output/batch",b,"rawdataoutput/attainedage_aspr_df_m_kk.csv"), row.names = F)
write.csv(attainedage_ICCC_aspr_m, paste0("~/GitHub/POSIM-prev/analysis/SA_inc_output/batch",b,"rawdataoutput/attainedage_ICCC_aspr_m_kk.csv"), row.names = F)
write.csv(survstatus_df, paste0("~/GitHub/POSIM-prev/analysis/SA_inc_output/batch",b,"rawdataoutput/survstatus_df_kk.csv"), row.names = F)
write.csv(survstatus_ICCC_m, paste0("~/GitHub/POSIM-prev/analysis/SA_inc_output/batch",b,"rawdataoutput/survstatus_ICCC_m_kk.csv"), row.names = F)

write.csv(surv_kk, paste0("~/GitHub/POSIM-prev/analysis/SA_inc_output/batch",b,"rawdataoutput/surv_kk.csv"), row.names = F)
write.csv(surv_kk_ICCC_5, paste0("~/GitHub/POSIM-prev/analysis/SA_inc_output/batch",b,"rawdataoutput/surv_kk_ICCC_5.csv"), row.names = F)
write.csv(surv_kk_5, paste0("~/GitHub/POSIM-prev/analysis/SA_inc_output/batch",b,"rawdataoutput/surv_kk_5.csv"), row.names = F)
